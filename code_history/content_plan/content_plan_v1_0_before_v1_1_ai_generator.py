"""Архив рабочей версии v1.0 перед внедрением AI-генератора.

Что нового в следующей версии:
- генерация семидневного контент-плана через OpenAI Responses API и GPT-5.6;
- структурированные поля для каждого дня: цель, тема, формат, ключевой тезис и CTA;
- безопасная обработка ошибок без сохранения неготового результата.

Что умеет текущая версия:
- создаёт фиксированный контент-план из пяти пунктов по одной теме;
- сохраняет, показывает, ищет, редактирует и удаляет планы;
- использует aiogram FSM для пошаговых сценариев.

Текущие ограничения:
- содержание строится по статическому шаблону и не учитывает аудиторию, цель и специфику ниши;
- план не разбит на семь дней и не имеет структурированных SMM-полей;
- OpenAI API не используется.

Почему версия сохранена:
- это последняя проверенная рабочая версия раздела до перехода к AI-генерации;
- архив позволяет восстановить прежнее поведение и сравнить его с версией v1.1.
"""
