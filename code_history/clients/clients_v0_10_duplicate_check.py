...
Описание:
Десятая версия раздела "Клиенты".

Что нового:
- Добавлена проверка на дубликаты клиентов.
- При добавлении нового клиента бот проверяет, есть ли уже клиент с таким именем.
- Если клиент уже существует, бот не добавляет его повторно.
...
Текущие ограничения:
- Проверка дубликатов работает только по имени клиента.
...
в котором бот защищает список клиентов от повторного добавления одинаковых имён.
"""
    ...
    if waiting_for_client_name:
        ...
        if client:
            if client_exists(new_client_name):
                await message.answer(
                    f"Клиент «{new_client_name}» уже существует."
                )
            else:
                ...
                await message.answer(
                    f"✅ Клиент изменён на «{new_client_name}»."
                )
        else:
